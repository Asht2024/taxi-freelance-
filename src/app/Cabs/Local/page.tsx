...
const LocalPage = () => {
  ...
  useEffect(() => {
    ...
    if(totaldistance >40){
      alert("your Totaldistance more than 40km then continue your booking in outstation one-way")
    }
  ...
  .map(car => {
    let calculatedprice = 0;
       if(pickupcity == Cities[0]){
          if(car.car_name == "Sedan"){
            if(totaldistance<=15){
               ...
               calculatedprice = 1250 + (totaldistance - 25)*car.local_price_per_km
            }
          }
            ...
            if(totaldistance<=15){
              ...
              calculatedprice = 2000 + (totaldistance - 25)*car.local_price_per_km
           }
          }
          else {
               calculatedprice = car.local_min_price + ( totaldistance - 5)*car.local_price_per_km
          }
       }
       ...
       else if(pickupcity == Cities[2] && dropcity == Cities[3] || dropcity == Cities[2] && pickupcity == Cities[3]){
        if(car.car_name == "Sedan"){
       ...
       else if(pickupcity == Cities[2] && dropcity == Cities[1] || pickupcity == Cities[6] && dropcity == Cities[1]){
        if(car.car_name == "Sedan"){
          calculatedprice = 2800
        }
        else if(car.car_name == "SUV"){
        ...
        <div className="p-6">
          ...
          <div className="mb-4">
            ...
            {car.calculated_price && (
              <p className="text-sm text-green-600 mt-1">All Inclusive Price</p>
            )}
          </div>
          ...
          <div className="flex justify-between items-center border-t border-gray-100 pt-4">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-gray-600">{car.luggage} Luggage</span>
            </div>
            <button onClick={() => handleonclick(car)} className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-lg transition-colors duration-300 flex items-center">
              Book Now
  ...
  {mycars.length === 0 && (
    <div className="text-center py-12">
      <p className="text-xl text-gray-500">No cars available matching your criteria</p>
    </div>
  )}
</div>
      </motion.div>
